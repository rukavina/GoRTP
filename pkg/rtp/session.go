...
func (rs *Session) WriteData(rp *DataPacket) (n int, err error) {
	...
	if strOut.streamStatus != active {
		log.Printf("Stream [%d] is not active, but [%d]", rp.Ssrc(), strOut.streamStatus)
		return 0, nil
	}
	...
	log.Printf("Writing packet to %d remotes", len(rs.remotes))

	totalBytes := 0
	// Check here if SRTP is enabled for the SSRC of the packet - a stream attribute
	for _, remote := range rs.remotes {
		log.Printf("Writing packet to %+v", remote)
		bytes, err := rs.transportWrite.WriteDataTo(rp, remote)
		if err != nil {
			return bytes, err
		}
		totalBytes += bytes
